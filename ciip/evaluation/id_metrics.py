...
class CromaNormalize(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # print dimension
        print(f"CromaNormalize input shape: {x.shape}, dtype: {x.dtype}")
...
SSL4EO_MODEL_TRANSFORMS: Dict[str, Callable[[torch.Tensor], torch.Tensor]] = {
    ...
    "llama3_ms_clip_base": T.Compose(
        [
            T.CenterCrop((224, 224)),
            T.Normalize(mean=[1924.863, 2184.553, 2340.936, 2671.402, 3240.082, 3468.412, 3563.244, 3627.704, 3416.714, 2849.625], std=[1201.092, 1219.943, 1397.225, 1400.035, 1373.136, 1429.17, 1485.025, 1447.836, 1471.002, 1365.307])
        ]
    ),
    "rcf_13ch": T.Compose(
        [
