...
echo "$(date) S1 | Streaming extract from $SOURCE_DIR into $EXTRACT_DIR"
...
echo "$(date) S1 | Worker sharding id $worker_id of $worker_count (source: $sharding_source)"
...
for idx in "${!tarballs[@]}"; do
    ...
    if ! flock -n "$lock_fd"; then
        ...
        echo "$(date) S1 | $chunk_name locked by another worker despite sharding; contention should be rare"
        exec {lock_fd}>&-
        continue
    fi

    if [ -f "$marker" ]; then
        echo "$(date) S1 | $chunk_name extracted by another worker after sharding; contention should be rare, skipping"
        flock -u "$lock_fd"
        exec {lock_fd}>&-
        rm -f "$lock_path"
        continue
    ...
    > "$inprog"
    echo "$(date) S1 | Extracting $tarball directly into $EXTRACT_DIR"
