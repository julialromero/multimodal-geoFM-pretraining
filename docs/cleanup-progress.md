...
| 4 — audited removal | **In progress** | `ciip/open_clip_train/transforms.py` and `ciip/dataset.py` are retained due concrete consumers. The owner-approved `clip/dataset.py` removal is implemented after finding no current or fetched-ref caller. | Continue assessing candidates independently and rerun applicable compatibility baselines before each removal. |
