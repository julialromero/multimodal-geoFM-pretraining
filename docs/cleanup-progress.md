...
| 4 — audited removal | **Initial candidate set complete** | The owner-approved `clip/dataset.py` removal is implemented. Concrete consumers preserved the transform and CIIP dataset modules; documented CROMA support and package markers are retained. | Evaluate future nominated candidates independently and rerun applicable compatibility baselines before each removal. |
